...
Dishost 환경변수에는 3개만 넣으면 됩니다.

- 봇 토큰 환경변수
- CLIENT_ID
- MONGODB_URI

서버 ID는 필요 없습니다. 명령어는 전역 커맨드로 등록됩니다.
관리자 역할, 티켓 카테고리, 로그 채널은 환경변수에 넣지 않고 디스코드 서버에서 `/티켓설정` 명령어로 지정하세요.

전역 커맨드는 등록 후 디스코드에 보이기까지 몇 분 걸릴 수 있습니다.
